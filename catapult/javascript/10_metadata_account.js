...
(async () => {
  ...
  console.log("scopedMetadataKey", byteKey.toHex());

  const metaDataTransation = await firstValueFrom(
    // 既存のメタデータ情報を検索する
    metaHttp
      .search({
        targetAddress: account_w.address,
        scopedMetadataKey: byteKey.toHex(),
        sourceAddress: account_w.address,
  ...
  const aggregteTransaction = AggregateTransaction.createComplete(
    ...
    [metaDataTransation.toAggregate(account_w.publicAccount)],
    networkType,
    []
