...
def namespace_registration():
    ...
    namespace_transaction = facade.transaction_factory.create(
        {
            "signer_public_key": account.public_key,
            "deadline": deadline,
            "type": "namespace_registration_transaction_v1",
            "registration_type": "root",
            "duration": 1000,  # 1000ブロックだけ有効だが、所有者としては最低が1ヶ月になる
            "name": namespace,
            "fee": fee,
        }
    )
    ...
    # 作成したトランザクションに署名する
    signature = facade.sign_transaction(account, namespace_transaction)
    json_payload = facade.transaction_factory.attach_signature(
        namespace_transaction, signature
    )
    data = loads(json_payload)
    transaction_hash = facade.hash_transaction(namespace_transaction)
    signed_transaction = {
        "SignedTransaction": {
            "payload": data["payload"],
            "hash": str(transaction_hash),
            "signerPublicKey": str(account.public_key),
            "type": namespace_transaction.type_.value,
